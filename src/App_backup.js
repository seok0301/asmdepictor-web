...
const example_list = ["", "ex20_strip", "pf_strip", "lsipc_strip", "lsipc", "perl_strip"];
...
function App() {
  ...
  const [isInstructionLong, setIsInstructionLong] = useState([]);
  ...
  // 하나의 inst받아서 function name response로 return.
  const GetFunctionPrediction = async (inst, idx) => {
    let split_inst = inst.split(' ');
    if (split_inst.length > 300) {
      let newIsInstructionLong = [...isInstructionLong];
      newIsInstructionLong[idx] = true;
      setIsInstructionLong(newIsInstructionLong);

      split_inst = split_inst.slice(0, 300)
      inst = split_inst.join(' ');
    }

    if (parseInt(selectModel) === 1) {
      const response = await fetch(`http://${IP}:30303/predictions/AsmDepictor`, {
        method: "POST",
        headers: {"Content-Type": "application/json",
        "Access-Control-Allow-Origin": "*"},
        body: JSON.stringify({'code': inst})
        })
        .then((res) => res.text())
      return response;
  ...
  const AnalysisButtonClick = async () => {
    ...
    let newIsInstructionLong = [];
    // instruction 긴 함수 표시를 위한 배열 초기화.
    for (let idx in result) {
      newIsInstructionLong.push(false);
    }
    setIsToggled(newIsInstructionLong);
    ...
    // result func에 추론된 function name 저장.
    for (let idx in result) {
      const prediction = await GetFunctionPrediction(result[idx].inst, idx);
      // result[idx].func = prediction.replace(/ /g, "_");
      result[idx].func = prediction;
    }
            ...
            <tbody>
              ...
              <tr>
                ...
                <td className={styles.text}>
                  ...
                  {isInstructionLong[idx] ? <span className={styles.inst_too_long}>* Instruction is too long, so it is truncated and used for prediction.</span> : ""}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <div>
        ...
        <div className={styles.wrapper}>
          <section>
            ...
            <label className={styles.upload_label} htmlFor="file">Upload</label>
          ...
          <section>
            ...
            <select value={selectExampleFile} onChange={SelectBinaryFile}>
          ...
          <section>
            ...
            <select value={selectModel} onChange={SelectModel}>
